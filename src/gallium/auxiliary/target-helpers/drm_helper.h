...
#ifdef GALLIUM_LJMGPU
#include "ljmgpu/drm/ljmgpu_drm_public.h"

static struct pipe_screen *
pipe_ljmgpu_create_screen(int fd, const struct pipe_screen_config *config)
{
   struct pipe_screen *screen;

   screen = ljmgpu_drm_screen_create(fd);
   return screen ? debug_screen_wrap(screen) : NULL;
}
DRM_DRIVER_DESCRIPTOR(ljmgpu, NULL, 0)

#else
DRM_DRIVER_DESCRIPTOR_STUB(ljmgpu)
#endif
